/**
 * VRT_LOGO_DATA_URI — real VRTSync brand asset embedded as a base64 data-URI
 * for server-side PDF generation (puppeteer cannot use Vite @assets aliases).
 *
 * Source: attached_assets/FINAL-02_1754876518960.png (710×215 RGBA, ~29 KB)
 * Same file the web app's powered-by footer references via @assets.
 *
 ...
 * If no candidate resolves, exports an empty string so the header degrades
 * gracefully to a text-only label with a logged warning.
 */
...
const ASSET_FILE = 'FINAL-02_1754876518960.png';
  ...
  for (const p of _candidates) {
    if (existsSync(p)) {
      try {
        ...
        console.warn(`[PDF] VRT logo read failed at ${p}:`, err instanceof Error ? err.message : err);
      }
    }
  }
  console.warn('[PDF] VRT logo asset not found — Repairs Summary header falls back to text-only label');
  return '';
}

export const VRT_LOGO_DATA_URI: string = _loadLogo();
