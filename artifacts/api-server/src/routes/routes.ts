...
import { registerEstimateRoutes, estimateOwnershipMatches } from "./estimate-routes";
import { registerApproveRoutes } from "./approve-routes";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // All /api/estimates* routes (CRUD, lifecycle, PDF, email, public
  // approval/rejection tokens, convert-to-work-order) live in
  // ./estimate-routes.ts — Task #640. registerRoutes contributes no
  // estimate routes of its own; it wires in the cross-cutting deps
  // (pricing-visibility strip from Task #643 and the lifecycle audit
  // emitter from Task #641) so the moved routes keep the same
  // behaviour they had inline.
...
console.log("Required redirect URI:", window.location.protocol + "//" + window.location.host + "/api/quickbooks/callback");
  ...
  //   All request body parameters are IGNORED. targetCompanyId is always the
  //   session company. The UPDATE target row is derived entirely server-side:
  //   (1) verify this company has no proper QB row, (2) verify exactly ONE stale
  //   row exists globally, (3) patch it. No client-supplied selector of any kind
  //   is used — a forged body cannot influence which row is patched or to which
  //   company it is reassigned.
  //
  // super_admin:
  //   body.targetCompanyId overrides session company (required when super_admin
  //   has no session-company context). body.realmIdOverrides: Record<realmId,
  //   companyId> supports multi-row repair when multiple stale rows exist.
  app.post("/api/quickbooks/connection/repair", requireAuthentication, async (req: any, res: Response) => {
    ...
    try {
      if (role !== "super_admin") {
        ...
        // Re-derive target server-side — same conditions as the detection endpoint.
        const [properRows, staleRows] = await Promise.all([
          db.execute<{ count: string }>(
            sql`SELECT COUNT(*)::text AS count FROM quickbooks_integration WHERE company_id = ${targetCompanyId}`
          ),
          db.execute<{ id: number; realm_id: string }>(
            sql`SELECT id, realm_id FROM quickbooks_integration WHERE company_id = realm_id LIMIT 2`
          ),
        ]);
        const properCount = parseInt(properRows.rows[0]?.count ?? "0", 10);

        if (properCount > 0) {
          // Already properly connected — nothing to repair.
        ...
        if (staleRows.rows.length > 1) {
          res.status(409).json({
            message: "Multiple stale QuickBooks rows detected. Contact your administrator to resolve this.",
            staleCount: staleRows.rows.length,
          });
          return;
        }

        // Exactly one stale row: patch it to this company.
        const staleRow = staleRows.rows[0];
        await db.execute(
          sql`UPDATE quickbooks_integration SET company_id = ${targetCompanyId}, updated_at = NOW() WHERE id = ${staleRow.id}`
        );
        logger.info({ rowsPatched: 1, targetCompanyId, realmId: staleRow.realm_id }, "POST /api/quickbooks/connection/repair complete");
        res.json({ ok: true, rowsPatched: 1, details: [{ id: staleRow.id, realmId: staleRow.realm_id, newCompanyId: targetCompanyId }] });
        return;
      }
      ...
      const targetCompanyId = String(raw);
      ...
      logger.info({ rowsPatched: details.length, targetCompanyId, details }, "POST /api/quickbooks/connection/repair complete (super_admin)");
  ...
  // The canonical replacements (PATCH approve/reject,
  // /submit-for-review, /send-approval-email, /resend) all live in
  // ./estimate-routes.ts as of Task #640.
  ...
  // Manager approval gate endpoints (Task #767 / #768)
  // Extracted to ./approve-routes.ts so they can be unit-tested without a
  // live DB. registerApproveRoutes mounts all four handlers:
  ...
  registerApproveRoutes(app, storage, requireAuthentication, {
    recordLifecycleAudit,
  });
