// Behavior tests for POST /api/estimates/:id/unapprove.
//
...
// The storage stub's `unapproveEstimate` method returns controlled
// outcomes, letting us test both success paths and the two 409
// variants (lifecycle mismatch — caught pre-storage — and WO
// progressed — thrown by storage as UnapproveEstimateConflictError).
...
async function post(
  ...
  const res = await fetch(`${url}/api/estimates/1/unapprove`, {
    method: "POST",
    headers,
    body: JSON.stringify({}),
  });
...
describe("POST /api/estimates/:id/unapprove — behavior", () => {
  ...
  it("200: company_admin with pending work order — deletes WO and flips estimate to sent", async () => {
    const { server: s, url } = makeApp({
      getCustomer: async () => stubCustomer,
      getEstimate: async () => approvedEstimate,
      createEstimateFromPayload: async () => { throw new Error("not used"); },
      updateEstimateWithItems: async () => { throw new Error("not used"); },
      unapproveEstimate: async () => ({
        estimate: sentEstimate as unknown as import("@workspace/db").Estimate,
        deletedWorkOrderId: 42,
      }),
    });
    server = s;
    ...
    assert.equal(body.deletedWorkOrderId, 42, "should report deleted WO id");
    assert.equal(
  ...
  it("200: super_admin with no linked work order — deletedWorkOrderId is null", async () => {
    const { server: s, url } = makeApp({
      getCustomer: async () => stubCustomer,
      getEstimate: async () => approvedEstimate,
      createEstimateFromPayload: async () => { throw new Error("not used"); },
    ...
    assert.equal(status, 200, `body: ${JSON.stringify(body)}`);
    assert.equal(body.deletedWorkOrderId, null, "should be null when no WO existed");
  });

  it("409: storage throws UnapproveEstimateConflictError(wo_progressed) — returns workOrderStatus + workOrderId", async () => {
    const { server: s, url } = makeApp({
      getCustomer: async () => stubCustomer,
      getEstimate: async () => approvedEstimate,
      createEstimateFromPayload: async () => { throw new Error("not used"); },
      ...
      unapproveEstimate: async () => {
        throw new UnapproveEstimateConflictError("wo_progressed", {
          workOrderId: 7,
          workOrderNumber: "WO-20260617-001",
    ...
    assert.equal(body.workOrderStatus, "assigned", "should surface WO status");
    assert.equal(body.workOrderId, 7, "should surface WO id");
    assert.ok(
      typeof body.message === "string" && body.message.includes("assigned"),
      "message should mention the status",
    );
  });

  it("409: estimate lifecycle is not approved (pre-storage check)", async () => {
    const notApproved = {
      ...approvedEstimate,
      lifecycle: "sent",
      status: "pending",
  ...
  it("403: billing_manager is not authorized to unapprove", async () => {
    const { server: s, url } = makeApp({
      getCustomer: async () => stubCustomer,
      getEstimate: async () => approvedEstimate,
      createEstimateFromPayload: async () => { throw new Error("not used"); },
  ...
  it("403: irrigation_manager is not authorized to unapprove", async () => {
    const { server: s, url } = makeApp({
      getCustomer: async () => stubCustomer,
      getEstimate: async () => approvedEstimate,
      createEstimateFromPayload: async () => { throw new Error("not used"); },
  ...
  it("403: field_tech is not authorized to unapprove", async () => {
    const { server: s, url } = makeApp({
      getCustomer: async () => stubCustomer,
      getEstimate: async () => approvedEstimate,
      createEstimateFromPayload: async () => { throw new Error("not used"); },
