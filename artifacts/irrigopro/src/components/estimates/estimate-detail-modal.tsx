...
// ── Zone-Grouped Estimate View ─────────────────────────────────────────────
// Renders the zone-grouped "Repairs Summary" + per-zone detail blocks that
// match the inspection-origin estimate PDF layout. Only shown for estimates
// whose items carry controller/zone tags (isInspectionOriginEstimate = true).
  ...
  return (
    <div className="rounded-lg border border-blue-100 overflow-hidden">
      <div className="flex items-center justify-between px-4 py-2.5 bg-blue-600 text-white">
        ...
        <span className="font-bold text-sm tabular-nums">{formatCurrency(group.zoneTotal)}</span>
      </div>
          ...
          <tbody>
            {group.items.map((item, idx) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="py-2 px-2 text-right text-gray-700">
                    {isLaborOnly ? <span className="text-gray-400">—</span> : formatCurrency(partPrice)}
                  </td>
                  <td className="py-2 px-2 text-right text-gray-700">
                    {isLaborOnly ? <span className="text-gray-400">—</span> : formatCurrency(partsTotal)}
                  </td>
                  <td className="py-2 px-2 text-right text-gray-700">
                    {isLaborOnly ? <span className="text-gray-400">—</span> : formatCurrency(partsTotal)}
                  </td>
            ...
            <tr className="border-t border-dashed border-gray-200 bg-gray-50 text-xs italic text-gray-600">
              <td colSpan={4} className="py-2 px-3">
                Zone labor · {group.laborHrs.toFixed(2)} hrs × {formatCurrency(laborRate)}/hr
              </td>
              <td className="py-2 px-2 text-right font-medium text-gray-800">
                {formatCurrency(zoneLaborAmt)}
              </td>
            </tr>
            {/* Subtotal row */}
            <tr className="border-t-2 border-blue-200 bg-blue-50">
              ...
              <td className="py-2 px-2 text-right font-bold text-blue-900 tabular-nums">
                {formatCurrency(group.zoneTotal)}
              </td>
            </tr>
          </tbody>
        </table>
...
function EstimateZoneGroupedView({
  items,
  laborRate,
  partsSubtotal,
  laborSubtotal,
              ...
              <tbody>
                {groups.map((g, idx) => (
                  ...
                  >
                    ...
                    <td className="py-2 px-2 text-right text-gray-700">{formatCurrency(g.partsTotal)}</td>
                    <td className="py-2 px-2 text-right text-gray-700">{g.laborHrs.toFixed(2)}</td>
                    <td className="py-2 pl-2 text-right font-semibold text-gray-900 tabular-nums">{formatCurrency(g.zoneTotal)}</td>
                  </tr>
                ))}
                <tr className="border-t-2 border-gray-300 bg-blue-50">
                  ...
                  <td className="py-2 px-2 text-right font-bold text-gray-900">{formatCurrency(partsSubtotal)}</td>
                  <td className="py-2 px-2 text-right font-bold text-gray-900">
                    {groups.reduce((s, g) => s + g.laborHrs, 0).toFixed(2)}
                  </td>
                  <td className="py-2 pl-2 text-right font-bold text-blue-900 tabular-nums">{formatCurrency(grandTotal)}</td>
                </tr>
              </tbody>
            </table>
          </div>
          ...
          <div className="flex justify-end pt-2">
            <div className="w-full max-w-xs space-y-1.5 text-sm">
              <div className="flex justify-between text-gray-600">
                ...
                <span className="font-medium tabular-nums">{formatCurrency(partsSubtotal)}</span>
              </div>
              <div className="flex justify-between text-gray-600">
                <span>
                  Labor ({totalLaborHours.toFixed(2)}h × {formatCurrency(laborRate)}/hr)
                </span>
              ...
              <div className="flex justify-between items-center border-t border-gray-200 pt-1.5">
                ...
                <span className="font-bold text-lg text-blue-900 tabular-nums">{formatCurrency(grandTotal)}</span>
              </div>
            </div>
          </div>
        </CardContent>
