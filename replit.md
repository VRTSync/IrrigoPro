...
Parts Catalog Access: Billing managers and irrigation managers now have complete access to the parts catalog with the same functionality as company administrators. This includes viewing all parts with pricing information, creating and editing parts, bulk import capabilities, advanced filtering and search, and QuickBooks integration for parts sync. Additionally, irrigation managers have access to both the full Parts Catalog and a simplified Parts List view through a dropdown navigation menu, providing flexibility for different use cases. The parts catalog provides full inventory management capabilities for all management-level personnel.
